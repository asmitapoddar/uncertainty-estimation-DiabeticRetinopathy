...
__all__ = ['ResNetDropout', 'resnet_dropout']
...
def resnet_dropout(pretrained=False, depth=18, num_classes=1000,**kwargs):
    ...
    if (pretrained):
        model = ResNetDropout(block, num_blocks)
        ...
        model = ResNetDropout(block, num_blocks, **kwargs)
    model.fc = nn.Linear(model.fc.in_features,num_classes)
    return model
